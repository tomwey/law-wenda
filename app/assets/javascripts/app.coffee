window.App =
  ...
  alert: (msg, to) ->
    $(to).before("<div class='alert alert-error'><a class='close' href='#' data-dismiss='alert'>X</a>#{msg}</div>")
  
  # 成功信息显示，to 显示在那个dom前(可以用 css selector)
  notice: (msg, to) ->
    $(to).before("<div class='alert alert-success'><a class='close' href='#' data-dismiss='alert'>X</a>#{msg}</div>")
